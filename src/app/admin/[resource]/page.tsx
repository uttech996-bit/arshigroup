...
export default async function AdminResourcePage({ params }: { params: Promise<{ resource: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-950 px-5 py-10 text-white sm:px-8"><div className="mx-auto max-w-7xl"><div className="flex flex-wrap items-center justify-between gap-4"><div><Link href="/admin" className="text-sm text-slate-400 hover:text-white">← Admin</Link><p className="mt-6 text-sm font-semibold uppercase tracking-[0.3em] text-blue-400">ARSHI GROUP · ADMIN</p><h1 className="mt-2 text-4xl font-bold">{config.title}</h1><p className="mt-2 text-slate-500">Search, filter and manage live records.</p></div>{resource !== "clients" && <Link href={`/admin/${resource}/new`} className="rounded-full bg-blue-600 px-5 py-2.5 text-sm font-semibold hover:bg-blue-500">+ Add new</Link>}</div>{error ? <div className="mt-8 rounded-2xl border border-red-500/20 bg-red-500/10 p-5 text-red-300">Unable to load records. Check admin access and database policies.</div> : <AdminResourceTable resource={resource} columns={[...config.columns]} rows={(rows ?? []) as Record<string, unknown>[]} />}</div></main>;
}
